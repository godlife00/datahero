                    ...
                    <div id="tab1" class="tab_content">
                      ...
                      <table cellspacing="0" border="1" class="tableRanking">
						...
						<?php $cnt=0; foreach($financial_status_adr as $key=>$val) :?>
						  ...
						  else if($cnt==9|| ($cnt>=17&&$cnt<=20)) $tClass=' class="depth03"';
						  ?>
                          <tr<?=$tClass;?>>
							...
							<?php foreach($history_mrt as $tVal) :?>
                            <!--<td><?=$tVal[$key]?></td>-->
							<td><?=($key=='sf1_accoci'||$key=='sf1_retearn') ? number_format($tVal[$key]/1000000) : $tVal[$key]?></td>
							<?php endforeach;?>
							<?php endif;?>
                          </tr>
						<?php $cnt++; endforeach;?>
						...
						<?php $cnt=0; foreach($financial_status as $key=>$val) :?>
						  <?php 
						  $tClass=''; 
						  if($cnt==0||$cnt==14||$cnt==26) $tClass=' class="depth01"';
						  else if($cnt>=2&&$cnt<=4) $tClass=' class="depth02"';
						  else if(($cnt>=11&&$cnt<=13)||($cnt>=21&&$cnt<=25)||$cnt==27) $tClass=' class="depth03"';
						  ?>
                          <tr<?=$tClass;?>>
							<?php if(strstr($key, 'sf1_null')) :?>
                            <td><span><strong><?=$val;?></strong></span></td>
							...
							<?php if($ticker_unit>0) :?>
                            <!--<td><?=number_format(str_replace(',','',$tVal[$key])/$ticker_unit);?></td>-->
							<td><?=($key=='sf1_accoci'||$key=='sf1_retearn') ? number_format((str_replace(',','',$tVal[$key])/$ticker_unit)/1000000) : number_format(str_replace(',','',$tVal[$key])/$ticker_unit)?></td>
							<?php else :?>
                            <!--<td><?=$tVal[$key]?></td>-->
							<td><?=($key=='sf1_accoci'||$key=='sf1_retearn') ? number_format($tVal[$key]/1000000) : $tVal[$key]?></td>
                            <?php endif;?>
						...
						<?php $st++; endif;?>
                          <tr>
							...
							<?php
								...
								if($row['sf1_revenue']=='0'|| $row['sf1_revenue']=='N/A') $dpval = 'N/A';
								//if(!is_numeric(str_replace('%','',abs($dpval)))||is_nan(str_replace('%','',abs($dpval)))||is_infinite(str_replace('%','',abs($dpval)))) $dpval = 'N/A';
                          ...
                          <table cellspacing="0" border="1" class="tableRanking">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="num"><span><?=(!is_numeric($mrt_data['sf1_opmargin']) || is_nan($mrt_data['sf1_opmargin']) || is_infinite($mrt_data['sf1_opmargin'])) ? 'N/A':$mrt_data['sf1_opmargin']?></span></td>
                                </tr>
                                <tr>
...
?>
                          ...
                          <table cellspacing="0" border="1" class="tableRanking">
                            ...
                            <tbody>
                                <tr>
                                    <th colspan="2" class="th_ft"><span>성장성(전년대비)</span><span class="day"><?=(!isset($orig_mrq_data['sf1_reportperiod'])||$orig_mrq_data['sf1_reportperiod']=='') ? 'N/A':date('y.m/d', strtotime($orig_mrq_data['sf1_reportperiod']))?></span></th>                                      
                                </tr>
                                <tr>
