...
class MY_Model extends CI_Model {
	...
	*/
	...
	public function getElasticList($params = array(), $extra = array()) {
		...
		$cmd = "curl -X GET 'http://localhost:9200/_sql' -H 'Content-Type: application/json' -d".escapeshellarg($query);
		//echo 'CMD IS ...  '.$cmd."\n";

		//echo $cmd."\n";
		$exec_result = shell_exec($cmd);
		//echo 'EXEC RESULT IS ... '.$exec_result;
		//echo "\n";
	...
	*/
	...
	protected function getWstreetDB($name) {
            $this->load->config('database');
            $allow_names = $this->config->item('wstreet_db_names');
            if(sizeof($allow_names) <= 0) {
                return $this->db;
            }
            ...
            if( ! in_array($name, $allow_names)) {
                if( ! IS_REAL_SERVER) {
                    echo 'Check Wstreet DB Name [ '.$name.' ]. (Allow DB names : '.implode(', ', $allow_names).')';
                    exit;
                }
                return $this->db;
            }

			if( ! isset($this->wstreet_db[$name])) {
				$this->wstreet_db[$name] = $this->load->database($name, true);
			}
			return $this->wstreet_db[$name];
	}
	...
	public function getCount($params=array()) {
		...
		else if(isset($params['wstreetdb']) && $params['wstreetdb'] == true) {
			$db = $this->getWstreetDB($params['wstreetdb']);
			$is_slave = true;
			unset($params['wstreetdb']);
		}
	...
	*/
	public function getList($params = array(), $extra = array(), $type='array') {
		...
		else if(isset($extra['wstreetdb']) && $extra['wstreetdb'] == true) {
			$db = $this->getWstreetDB($extra['wstreetdb']);
			$is_slave = true;
			unset($extra['wstreetdb']);
		}
	...
	public function doUpdate($pk, $data_params, $skip_escape=array(), $exec_type='exec') {
		...
		if(strlen($this->elastic_table) > 0) {
			if($this->getData() == 1) {		// affected_row 가 0 이상
				$el_res = $this->saveElastic($pk);
				//print_r($el_res);
				//$this->setSuccessResult($el_res);
			}
		}
	...
	*/
	...
	public function saveElastic($ids, $data=array()) {
		...
		foreach($rows as $row) {
			...
			$cmd = ("/usr/local/bin/curl -XPOST ".escapeshellarg(ELASTIC_HOST."/".$this->elastic_table."/".$row[$this->pk])." -H 'Content-type: application/json' -d ".escapeshellarg(json_encode($params)));
			echo $cmd."\n";
