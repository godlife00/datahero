...
$db['quandl'] = array(
        ...
        'username' => 'quandl',        // db user ID
        'password' => 'quandldb',    // db user Password
        'database' => 'quandl',    // 커맨드 실행시 첫번째 파라메터가 $db['xxxx']['database'] 에 들어 있는 값이어야 함.
);
...
$elastic_tables['elastic_sf1_tb'] = array(
    'query'             => 'select * from sf1_tb',    // 데이터 리스트 쿼리. join문도 가능. (겹치는 필드는 alias로 안겹치게.)
    'tables'            => array('sf1_tb'),           // 쿼리문에서 다루고있는 테이블명들.
    'unserialize'       => array(),
    'skip_fields'       => array(),
    'strip_tags'        => array(),
    'pk_field_name'     => 'sf1_id',                      // 쿼리 데이터 중 PK값. 엘라스틱서티 데이터의 pk로 사용됨.
);
$elastic_tables['elastic_sep_tb'] = array(
    'query'             => 'select * from sep_tb',    // 데이터 리스트 쿼리. join문도 가능. (겹치는 필드는 alias로 안겹치게.)
    'tables'            => array('sep_tb'),           // 쿼리문에서 다루고있는 테이블명들.
    'unserialize'       => array(),
    'skip_fields'       => array(),
    'strip_tags'        => array(),
    'pk_field_name'     => 'sep_id',                      // 쿼리 데이터 중 PK값. 엘라스틱서티 데이터의 pk로 사용됨.
);
