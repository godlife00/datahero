                ...
                <span class="<?=($cell_type=='ratio') ? 'fix-active' : ''?>" onclick="manager.setCellType('ratio', this)">구성비율</span>
                    ...
                    <div class="standard top"><!-- 최상단에 위치한 경우 레이아웃용 class = top -->                 
                        ...
                        <div class="select open" style="width:110px;">
                            ...
                            <button type="button" class="my_value"><?php if(isset($ticker_currency) && $ticker_currency) echo '* 단위 : '.$ticker_currency;?></button>
    ...
    ?>
        ...
        // * 이자비용은 손익계산서 기준, 주석에 있는 값은 미표시 => 표출조건1
        ...
        ?>
                    ...
                    <?php $t=0; $max=0; foreach(array_keys($data) as $yyyymm) : ?>
                    <?php $max++; if($max>41) break;?>
                    <th scope="col"><span><?=strlen($yyyymm)>8 ? date('y.m/d', strtotime(str_replace('.','-',$yyyymm))) : ''?></span></th>
                    <?php $t++; endforeach; ?>
                    <?=($t==1)?'<th>&nbsp;</th>':''?>
            ...
            <tr class="<?=$tr_depth_class?>">
                ...
                <?php $col++;?>
                <?php if($col>41) break;?>
                <td<?=!isset($row[$key]) ? ' class="na"':''?>><?php
                if(isset($row[$key])) {
                    if($ticker_unit>0) {
                        echo number_format(str_replace(',','',$row[$key])/$ticker_unit);
                    }
                    else {
                        echo $row[$key];
                    }
...
$(document).ready(function(){
    ...
    //parent.setMenu('financials') ; 
    window.parent.postMessage({ childData : ' financials ' }, '*'); 
});
</script>