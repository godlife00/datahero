...
require_once dirname(__FILE__).'/sn_base_pc.php';
class Sn_stocks extends Sn_BasePC_Controller{
	...
	{
        $this->common->locationhref('/sn_stocks/summary');
		exit;
	}
...
*/
    ...
    public function ajax_save_search_history() {
        ...
        if(isset($this->ticker_info_map[$ticker])) {
			...
			$search_url = CS_URL.'/main/ajax_save_search_history/S?ticker='.$ticker;
