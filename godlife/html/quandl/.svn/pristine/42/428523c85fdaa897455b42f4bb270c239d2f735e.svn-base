...
class Common {
	...
	function getCurrency() {
		...
		if( ! is_file(WEBDATA.'/currency.info') || filemtime(WEBDATA.'/currency.info') < time()-3600 or true) {
			touch(WEBDATA.'/currency.info');
			$result = explode('|', str_replace(',','',file_get_contents('http://www.itooza.com/stock/vExchange.htm')));
			if(sizeof($result) != 4) {
				$res = unserialize(file_get_contents(WEBDATA.'/currency.info'));
				return $res['currency'];
			}
			list($date, $currency, $sign, $rate) = $result;

			if(is_numeric($currency) && floatval($currency) > 500) {
				$cache = array(
					'date' => $date,
					'currency' => $currency,
					'sign' => $sign,
				...
				file_put_contents(WEBDATA.'/currency.info', serialize($cache));
				return $currency;
			} else {
				// todo. 에외 알림
			}
		...
		$result = unserialize(file_get_contents(WEBDATA.'/currency.info'));
		return floatval($result['currency']);
	}
	...
	function getDataByPK($arr, $key='idx') {
		...
		foreach($arr as $item) {
			if(is_array($item)) {
                if( ! IS_REAL_SERVER && ! isset($item[$key])) {
                    print_r($arr);
                    echo $key.' key is not exists.';
                    continue;
                }
	...
	function logWrite($filepath, $logdata, $filename='') {
		$path = $_SERVER['DOCUMENT_ROOT']."/logdata";
		$ymd = date("Ymd", time());
	...
	// CURL 함수
	function restful_curl($url, $param='', $method='POST', $timeout=30) {
    ...
    function get_sync_info($ticker, $lastpricedate=0) {
		$file_path = str_replace('hoon','datahero',WEBDATA).'/current_data/'.$ticker;
		$file_path = str_replace('quandl','wallstreet',$file_path);

        $item = unserialize(file_get_contents($file_path));
