...
class Op_stocks extends BasePC_Controller{
	...
	private function _getBaseData($ticker_code, $dimension='MRT', $cell_type='data', $pExtra=array()) {
        ...
        if(
            ...
            $this->common->locationhref('/op_stocks/summary');
            exit;
        }
	...
	public function summary($ticker='') {
		...
		$this->op_view('stocks/op_summary', $data);
	}
	public function invest($ticker='') {
        ...
        if( ! $is_ajax) {
		    $this->op_view('stocks/op_invest', $data);
        } else {
            $content = $this->op_view('stocks/op_invest', $data, true);
	...
	public function financials($ticker='') {
		...
		$data['data'] = $this->historylib->convertRows($data['data'], $extra);
        if( ! $is_ajax) {
		    $this->op_view('stocks/op_financials', $data);
        } else {
            $content = $this->op_view('stocks/op_financials', $data, true);
            list($head, $result, $tail) = explode('<!-- TABLE SCROLL DIV -->', $content, 3);
            echo $result;
        }
	}
	public function competitors($ticker='') {
		...
		$this->op_view('stocks/op_competitors', $data);
	}
	...
	public function trends() {
		...
		$this->op_view('stocks/op_trends', $data);
	}
	...
	public function performance($page = '1') {
		...
		// 페이징
		$config['base_url'] = '/op_stocks/performance';
		...
		$this->op_view('stocks/op_performance', $data);
	}
	...
	public function financial_chart($ticker='', $dimension='MRY') {
		...
		$this->op_view('stocks/op_vichart', $data);
	}
	...
	public function chart($ticker) {
		...
		//$data['like'] = $this->_like($ticker);	
		//$data['opinion'] = $this->_opinion($ticker);
		$this->op_view('stocks/chart', $data);
	}
