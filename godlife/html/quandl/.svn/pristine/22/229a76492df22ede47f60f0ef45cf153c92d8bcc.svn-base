                ...
                <table cellspacing="0" border="1" class="tableColtype typeOrder competitorsTable">
                    ...
                    <?php 
                        ...
                        foreach(array_keys($field_title_map) as $field) : 
                            ...
                            ?>
                            <td><a href="/wm_stocks/summary/<?=$row['tkr_ticker']?>" class="drop" <?=$style?>><?=isset($row['cp_korname']) ? $row['cp_korname'] : $row['tkr_name']?></a><span class="eng"><?=$row['tkr_name']?></span></td>
                            <?php
                                }
                                else {
                                ?>
...
$(document).ready(function(){
    ...
    //parent.setMenu('competitors') ; 
    window.parent.postMessage({ childData : ' competitors ' }, '*'); 
});
</script>