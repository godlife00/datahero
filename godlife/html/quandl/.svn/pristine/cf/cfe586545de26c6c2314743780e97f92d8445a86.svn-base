...
class Manage extends BaseAdmin_Controller{
	...
	public function set_format() {
		...
		$data = array();
		$data['tabs'] = $this->historylib->getGridFilenames();
		$this->_view('manage/set_format', $data);
	}

	public function set_format_process() {
		$this->load->model('business/historylib');
		$request = $this->input->post();
		$allows = $this->historylib->getGridFilenames();
		...
		foreach($request as $k => $content) {
			...
			if( ! in_array($k, $allows)) {
				echo $k.' is not exist field!';
				exit;
			}
			$filepath = APPPATH.'/models/business/'.$k;
			if(is_file($filepath)) {
				...
				$this->common->logWrite('set_format', print_r(array(
					'file' => $filepath,
					'before' => $old_content,
					'after' => $content
				), true), $k);
		...
		$this->common->locationhref('/adminpanel/manage/set_format');
	}
