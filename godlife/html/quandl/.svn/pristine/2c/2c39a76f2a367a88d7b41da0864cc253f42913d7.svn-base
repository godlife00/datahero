		...
		return this.each(function(){
			...
			$.each($t.p.colModel,function(ci){
				...
				if(this.search){
					...
					if(this.stype == 'between') { // by hamt.
						select = "<a title='"+st+"' style='padding-right: 0.5em;' soper='ge' class='soptclass' colname='"+this.name+"_from'>"+sot+"</a>";
						$('tr:eq(0) td:eq(0)', stbl).attr('colindex', ci+'_from').append(select).hide();
						select = "<a title='"+st+"' style='padding-right: 0.5em;' soper='le' class='soptclass' colname='"+this.name+"_to'>"+sot+"</a>";
					...
					if(soptions.clearSearch) {
						...
						$("tr:eq(0) td:eq(2)",stbl).append("<a title='"+csv+"' style='padding-right: 0.3em;padding-left: 0.3em;' class='clearsearchclass'>"+p.resetIcon+"</a>");
						if(this.stype == 'between') { // by hamt.
							$("tr:eq(1) td:eq(2)",stbl).append("<a title='"+csv+"' style='padding-right: 0.3em;padding-left: 0.3em;' class='clearsearchclass'>"+p.resetIcon+"</a>");
						}
					} else {
					...
					case "between": // by hamt.
						...
						if(this.stype == 'text') {
							$("tr:eq(0) td:eq(1)",stbl).append("<input type='text' style='width:100%;padding:0px;' name='"+(cm.index || cm.name)+"' id='gs_"+cm.name+"' value='"+df+"' "+this.attr+" />");
						} else if(this.stype == 'between') { // by hamt.
							$("tr:eq(0) td:eq(1)",stbl).append("<input type='text' placeholder='From' style='width:100%;padding:0px;' name='"+(cm.index || cm.name)+"_from' id='gs_"+cm.name+"_from' value='"+df+"' "+this.attr+" />");
							$("tr:eq(1) td:eq(1)",stbl).append("<input type='text' placeholder='To' style='width:100%;padding:0px;' name='"+(cm.index || cm.name)+"_to' id='gs_"+cm.name+"_to' value='"+df+"' "+this.attr+" />");
						}
						$(thd).append(stbl);

						if(soptions.attr) {$("input",thd).attr(soptions.attr);}
						//$.jgrid.bindEv.call($t, $("input",thd)[0], soptions);
						$.jgrid.bindEv.call($t, $("input",thd), soptions);
						if(p.autosearch===true){
							if(p.searchOnEnter) {
