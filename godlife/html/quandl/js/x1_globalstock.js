...
/*****
    function doSearch(obj) {
        ...
        for (var tic in search_ticker_list) {
            ...
            if (ticker == search) {
                // ticker 일치
                match_li = $('<li><a href="/x1_stocks/summary/' + tic + '"><span class="schCode">' + tic + '</span><span class="schList">' + search_ticker_list[tic].name.replace(search_obj,'<strong>'+search_obj+'</strong>') + '</span></a></li>');
                continue;
            }
            if ((ticker + search_ticker_list[tic].name).toUpperCase().indexOf(search) >= 0) {
                find_lis.push($('<li><a href="/x1_stocks/summary/' + tic + '"><span class="schCode">' + tic + '</span><span class="schList">' + search_ticker_list[tic].name.replace(search_obj,'<strong>'+search_obj+'</strong>') + '</span></a></li>'));
            }
        }
        if (match_li !== null) {
            ul.append(match_li);
...
function onSearchTicker(ticker) {
	...
	location.href = '/x1_stocks/summary/' + ticker;
}
...
function setSearchHistory(keyword) {
	...
	*/
	...
	$.ajax({
		url: '/x1_stocks/ajax_save_search_history',
		type: 'GET',
		data: { 'ticker': keyword },
		async: false,
		success: function () {
